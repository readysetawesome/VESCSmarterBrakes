...
class VESCSmarterBrakes
{
  public:
    ...
    void TurnOn();
    // newData: true only when a fresh VESC Status 1 CAN frame was received this loop.
    // Braking logic is gated on newData so the consecutive-measurement counter
    // reflects actual VESC updates, not loop iterations.
